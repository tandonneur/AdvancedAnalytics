...
class forest_regressor(object):
    ...
    def display_metrics(rf, X, y):
        ...
        print("{:.<23s}{:>9s}".format('Split Criterion', rf.criterion))
        if depth == None:
    ...
    def display_split_metrics(rf, Xt, yt, Xv, yv):
    ...
    def display_importance(rf, col, top='all', plot=False):
        nx = rf.n_features_
        if nx != len(col):
            print("NX=", nx)
        ...
        this_col = []
        for i in range(nx):
            features.append(rf.feature_importances_[i])
        ...
        if len(rf.classes_) == 2:
            ...
            for i in range(n_classes):
                if type(rf.classes_[0]) == str:
                    ...
                    print(fstr1.format('Class ', rf.classes_[i]), end="")
                    
            print("")
            for i in range(n_classes):
                if type(rf.classes_[0]) == str:
                    print(fstr2.format(str(i)+" "+rf.classes_[i]), end="")
                else:
                    print(fstr2.format('Class ', rf.classes_[i]), end="")
